...
if (!empty($_REQUEST['EmpID'])) {
  ...
  if ($_REQUEST['EmpID'] != "0") {
    $WHERE1 = "AND C.SaleEmployeeCode IN (SELECT DISTINCT CASE WHEN EmployeeCodeLV1 IS NULL THEN EmployeeCodeLV2 ELSE  EmployeeCodeLV1 END AS EmployeeCodeLV1   FROM [TSRData_Source].[dbo].[vw_EmployeeDataParent]  WHERE EmployeeCodeLV2 = '".$_REQUEST['EmpID']."' OR EmployeeCodeLV3 = '".$_REQUEST['EmpID']."' OR EmployeeCodeLV4 = '".$_REQUEST['EmpID']."' OR EmployeeCodeLV5 = '".$_REQUEST['EmpID']."' OR EmployeeCodeLV6 = '".$_REQUEST['EmpID']."' OR ParentEmployeeCode = '".$_REQUEST['EmpID']."')";
  }else {
    $WHERE1 = "AND C.SaleEmployeeCode = '".$_REQUEST['EmpID']."' ";
  }
}
        ...
        <div class="col-md-2">
          <div class="form-group group-sm">
            ...
            <select class="form-control select2 group-sm" name="LvEmp" id = "LvEmp">
              <option id="Lv_list" value="7">ทั้งหมด</option>
            </select>
          -->
              <?PHP
              $sql_case = "SELECT TOP 1 PositionLevel FROM [Bighead_Mobile].[dbo].[Position] WHERE PositionID in (SELECT PositionCode FROM Bighead_Mobile.dbo.EmployeeDetail WHERE EmployeeCode = '".$EmpID['0']."') AND SourceSystem = 'Sale' ORDER BY PositionLevel DESC";
