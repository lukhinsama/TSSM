        ...
        <div class="col-xs-12">
          ...
          <div class="box-body">
            ...
            <tbody>
              ...
              $sql_select = "SELECT $top row_number() OVER (ORDER BY ReceiptCode ASC) AS rownum,ReceiptCode,CONVERT(varchar(20),R.DatePayment,105) +' '+ CONVERT(varchar(5),R.DatePayment,108) as PaymentDueDate ,case when count(ReceiptCode) = 1 then convert(varchar,min(S.PaymentPeriodNumber)) else convert(varchar,min(S.PaymentPeriodNumber)) end as PaymentPeriodNumber,c.CONTNO AS CONTNO,CustomerName,Sy.Amount AS PAYAMT,R.TotalPayment AS PAYAMTS  ,Em.EmpID,case when ed.SaleCode is null then '-' else ed.SaleCode end as SaleCode, Ed.FirstName + ' ' + Ed.LastName AS Names";
