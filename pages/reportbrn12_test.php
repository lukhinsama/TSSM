...
if (($_COOKIE['tsr_emp_permit'] == 4 )) {
  if (substr($_COOKIE['tsr_emp_id'],0,1) == "0") {
    ...
    $WHERE .= "AND R.ZoneCode IN (
      SELECT DISTINCT Salecode  FROM [TSRData_Source].[dbo].[vw_EmployeeDataParentBRN] WHERE (EmployeeCodeLV2 = '".$EmpID['0']."' OR EmployeeCodeLV3 = '".$EmpID['0']."'   OR EmployeeCodeLV4 = '".$EmpID['0']."' OR EmployeeCodeLV5 = '".$EmpID['0']."' OR EmployeeCodeLV6 = '".$EmpID['0']."' OR ParentEmployeeCode = '".$EmpID['0']."')  )
 ";
  ...
  if (!empty($_REQUEST['EmpID'])) {
    ...
    if ($_REQUEST['EmpID'] == "7") {
      ...
      //คิวรี่ ค่า EmpID ที่ส่งเข้ามาว่ามีพักแรมหรือไม่
    $con = connectDB_BigHead();

    $sql = "SELECT teamSaleCode FROM TSRData_Source.dbo.TSSM_SaleOuting WHERE CAST('".DateEng($_REQUEST['startDate'])."' AS DATE) BETWEEN StartDate AND EndDate AND CAST('".DateEng($_REQUEST['endDate'])."' AS DATE) BETWEEN StartDate AND EndDate AND Active = 1 AND BranchCode = '".$_REQUEST['EmpID']."'
    ";

    $stmt = sqlsrv_query($con,$sql);
    if ($row = sqlsrv_fetch_array( $stmt, SQLSRV_FETCH_ASSOC)) {

      $WHERE1 = "AND ( C.salecode IN (SELECT DISTINCT SaleCode FROM [TSRData_Source].[dbo].[vw_EmployeeDataParentBRN]  WHERE (EmployeeCodeLV2 = '".$_REQUEST['EmpID']."' OR EmployeeCodeLV3 = '".$_REQUEST['EmpID']."' OR EmployeeCodeLV4 = '".$_REQUEST['EmpID']."'
        OR EmployeeCodeLV5 = '".$_REQUEST['EmpID']."' OR EmployeeCodeLV6 = '".$_REQUEST['EmpID']."' OR ParentEmployeeCode = '".$_REQUEST['EmpID']."'))
        OR C.SaleCode IN ( SELECT DISTINCT SaleCode FROM [TSRData_Source].[dbo].[vw_EmployeeDataParent] WHERE TeamCode = '".$row['teamSaleCode']."' ))";
  }else {
    //$BranchCode = "0";
      $WHERE1 = "AND C.salecode IN (SELECT DISTINCT SaleCode FROM [TSRData_Source].[dbo].[vw_EmployeeDataParentBRN]  WHERE (EmployeeCodeLV2 = '".$_REQUEST['EmpID']."' OR EmployeeCodeLV3 = '".$_REQUEST['EmpID']."' OR EmployeeCodeLV4 = '".$_REQUEST['EmpID']."'
        OR EmployeeCodeLV5 = '".$_REQUEST['EmpID']."' OR EmployeeCodeLV6 = '".$_REQUEST['EmpID']."' OR ParentEmployeeCode = '".$_REQUEST['EmpID']."')  )";
 ...
 ?>
  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
      <div class="row">
        <form role="form" data-toggle="validator" id="formSearch" name="formSearch" method="post" action="index.php?pages=reportbrn12_test">
        <div class="col-md-2">
          <h4>
            เก็บเงินงวดแรกรวม
          </h4>
          ...
          <!--<div class="box-body">-->
            ...
            <tbody>";
              ...
              $sql_case = "SELECT DISTINCT '".$_COOKIE['tsr_emp_name']."' AS PrintName,'".$searchDate."' AS printHead, R.ReceiptCode,R.DatePayment AS Paydate,R.DatePayment,CONVERT(varchar(20),R.DatePayment,105) +' '+ CONVERT(varchar(5),R.DatePayment,108) as PaymentDueDate,R.PaymentPeriodNumber,C.ContractReferenceNo,C.CONTNO,DC.CustomerName,R.ZoneCode,R.CreateBy AS EmpID,E.FirstName+' '+E.LastName AS EmpName,R.NetAmount,R.TotalPayment AS PAYAMT,R.PaymentComplete,CASE WHEN R.TotalPayment != 0 THEN CASE WHEN R.PaymentComplete = 1 AND NetAmount = TotalPayment THEN 'ส่งครบ' WHEN PaymentComplete = 0 THEN 'ส่งไม่ครบ' ELSE 'ส่งบางส่วนครบ' END ELSE 'ยกเลิกใบเสร็จ' END AS CompleteStatus,R.PrintOrder,ISNULL(R.PayDateOld,'-') AS PayDateOld ,ISNULL(B.BookNo,'-') AS BookNo ,ISNULL(B.ReceiptNo,'-') AS ReceiptNo
                FROM TSRData_Source.dbo.vw_ReceiptWithZone_ALL AS R INNER JOIN Bighead_Mobile.dbo.Contract AS C ON R.RefNo = C.RefNo INNER JOIN Bighead_Mobile.dbo.DebtorCustomer AS DC ON C.CustomerID = DC.CustomerID INNER JOIN Bighead_Mobile.dbo.Employee AS E ON R.CreateBy = EmpID LEFT JOIN Bighead_Mobile.dbo.MigrateReportDailyReceiptB AS B ON B.InvNo = R.ReceiptCode WHERE R.DatePayment BETWEEN CAST('".DateEng($_REQUEST['startDate'])." 00:00' AS datetime) AND CAST('".DateEng($_REQUEST['endDate'])." 23:59' AS datetime) AND TypeCode = 1 $WHERE1  ORDER BY R.DatePayment";
