...
if (!empty($_POST['printID'])) {
    ...
    $ch = curl_init();
    curl_setopt($ch, CURLOPT_URL,"http://app.thiensurat.co.th/tsr_ASP_tsr_SaleAdvance/webservice.asmx/Insert_AddReceiptCode");
    curl_setopt($ch, CURLOPT_POST, 1);
    curl_setopt($ch, CURLOPT_POSTFIELDS, "empid=$EmpID&datalist=$ReceiptCode&splt=$splt");
          ...
          <div class="form-group group-sm">
            <select class="form-control select2 group-sm" name="EmpID" >
              <optgroup label="พนักงานเก็บเงิน">
                <?php
                $EmployeeCode = "A".substr($_COOKIE['tsr_emp_id'],1,5);
                  if (($_COOKIE['tsr_emp_id'] == '031218') || ($_COOKIE['tsr_emp_id'] == '031584') || ($_COOKIE['tsr_emp_id'] == '031584')) {
                    $sql_case = "SELECT SaleCode as mcode,EmployeeName AS Name ,EmployeeCode AS EmpID,case when SaleCode is null then '-' else SaleCode end as SaleCode ,SupervisorCode FROM Bighead_Mobile.dbo.EmployeeDetail WHERE  salecode is not null AND SupervisorCode is not null AND EmployeeCode = '".$EmployeeCode."' ORDER BY mcode";
        ...
        <div class="col-xs-12">
          ...
          <div class="box-body">
            ...
            <tbody>
              ...
              $sql_body = " FROM TSRData_Source.dbo.vw_ReceiptWithZone AS R WITH(NOLOCK) LEFT JOIN Bighead_Mobile.dbo.Contract AS C WITH(NOLOCK) ON R.RefNo = C.RefNo
              LEFT JOIN TSRData_Source.dbo.vw_DebtorCustomer AS GC WITH(NOLOCK) ON C.CustomerID = GC.CustomerID
              ...
              WHERE $WHERE AND S.SalePaymentPeriodID = Sy.SalePaymentPeriodID AND Sy.Amount > 0
              --AND R.TypeCode = 0
              ) as result GROUP BY ReceiptCode,PaymentDueDate,CONTNO,CustomerName,EmpID
              ,SaleCode,Names,Paydate,PrintName,PaymentAmount ORDER BY ReceiptCode";
