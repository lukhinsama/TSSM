  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
      <div class="row">
        <div class="col-md-3">
          <h4>
            เก็บเงินงวดแรกรายวัน(บางส่วน)
          </h4>
        </div>
        <div class="col-md-3">
          <form role="form" data-toggle="validator" id="formSearch" name="formSearch" method="post" action="index.php?pages=reportoper12">
          ...
          <?php
           ...
           <div class="box box-info">
             <div class="box-header with-border">
               <B>รายการเก็บเงินงวดแรก(บางส่วน) ประจำวันที่ <?=$searchDate;?></B>
             </div>
...
*/
                            ...
                            FROM tsrdata_source.dbo.vw_ReceiptWithZone as Receipt INNER JOIN SalePaymentPeriodPayment ON Receipt.ReceiptID = SalePaymentPeriodPayment.ReceiptID INNER JOIN SalePaymentPeriod ON SalePaymentPeriodPayment.SalePaymentPeriodID = SalePaymentPeriod.SalePaymentPeriodID
                            WHERE $WHERE AND TotalPayment > 0 AND PaymentComplete = 0
                            GROUP BY Receipt.ReceiptCode, Receipt.TotalPayment, Receipt.RefNo, Receipt.DatePayment, Receipt.CreateBy,ZoneCode,PaymentComplete)
                            AS vw_PaymentSummary
          ...
          <div class="box box-info">
            <div class="box-header with-border">
              <P><center><H4>รายงานการเก็บเงินงวดแรก(บางส่วน)</H4></center><center><B><?=$teamcode[1]?> ประจำวันที่ <?=$searchDate;?></B></center></P>
            </div>
