...
if (($_COOKIE['tsr_emp_permit'] == 4 )) {
  if (substr($_COOKIE['tsr_emp_id'],0,1) == "0") {
    ...
    $WHERE .= "AND R.ZoneCode IN (
      SELECT DISTINCT Salecode  FROM [TSRData_Source].[dbo].[vw_EmployeeDataParentBRN] WHERE (EmployeeCodeLV2 = '".$EmpID['0']."' OR EmployeeCodeLV3 = '".$EmpID['0']."'   OR EmployeeCodeLV4 = '".$EmpID['0']."'
 ...
 ";
  ...
  if (!empty($_REQUEST['EmpID'])) {
    ...
    /*
      ...
      */
      $WHERE1 = "AND R.ZoneCode IN (SELECT DISTINCT Salecode FROM [TSRData_Source].[dbo].[EmployeeDataParent_all] WHERE StatusType = 'BRN' AND (EmployeeCodeLV2 = '".$_REQUEST['EmpID']."' OR EmployeeCodeLV3 = '".$_REQUEST['EmpID']."'
        OR EmployeeCodeLV4 = '".$_REQUEST['EmpID']."' OR EmployeeCodeLV5 = '".$_REQUEST['EmpID']."'
        OR EmployeeCodeLV6 = '".$_REQUEST['EmpID']."'
        OR ParentEmployeeCode = '".$_REQUEST['EmpID']."')
      UNION ALL SELECT DISTINCT SaleCode FROM [TSRData_Source].[dbo].[EmployeeDataParent_ALL_BACKUP_20180604] WHERE StatusType = 'BRN' AND (EmployeeCodeLV2 = '".$_REQUEST['EmpID']."' OR EmployeeCodeLV3 = '".$_REQUEST['EmpID']."'
 ...
 ?>
  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
      <div class="row">
        <form role="form" data-toggle="validator" id="formSearch" name="formSearch" method="post" action="index.php?pages=reportbrn12">
        <div class="col-md-2">
          <h4>
            เก็บเงินงวดแรกรวม
          </h4>
        ...
        <div class="col-xs-12">
          ...
          <!--<div class="box-body">-->
            ...
            <tbody>";
              ...
              $sql_case = "SELECT DISTINCT '".$_COOKIE['tsr_emp_name']."' AS PrintName,'".$searchDate."' AS printHead, R.ReceiptCode
              ,R.DatePayment AS Paydate,R.DatePayment,CONVERT(varchar(20),R.DatePayment,105) +' '+ CONVERT(varchar(5),R.DatePayment,108) as PaymentDueDate,R.PaymentPeriodNumber,C.ContractReferenceNo,C.CONTNO
              ,DC.CustomerName,R.ZoneCode,R.CreateBy AS EmpID,E.FirstName+' '+E.LastName AS EmpName,R.NetAmount
              ...
              ,R.PaymentComplete,CASE WHEN R.TotalPayment != 0 THEN CASE WHEN R.PaymentComplete = 1 AND NetAmount = TotalPayment THEN 'ส่งครบ' WHEN PaymentComplete = 0 THEN 'ส่งไม่ครบ' ELSE 'ส่งบางส่วนครบ' END ELSE 'ยกเลิกใบเสร็จ' END AS CompleteStatus
              ,R.PrintOrder,ISNULL(R.PayDateOld,'-') AS PayDateOld
              ...
              ,ISNULL(B.ManualVolumeNo,'-') AS BookNo ,ISNULL(B.ManualRunningNo,'-') AS ReceiptNo
                ...
                LEFT JOIN Bighead_Mobile.dbo.ManualDocument AS B ON B.DocumentNumber = R.ReceiptID AND B.isActive = 1 WHERE R.DatePayment BETWEEN CAST('".DateEng($_REQUEST['startDate'])." 00:00' AS datetime) AND CAST('".DateEng($_REQUEST['endDate'])." 23:59' AS datetime) AND TypeCode = 1 $WHERE1 ORDER BY R.DatePayment";
