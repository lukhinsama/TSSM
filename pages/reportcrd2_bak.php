...
if (($_COOKIE['tsr_emp_permit'] == 4 )) {
  if (substr($_COOKIE['tsr_emp_id'],0,1) == "0") {
    ...
    $WHERE .= "AND R.ZoneCode IN (
      ...
      WHERE StatusType = 'CRD' AND (EmployeeCodeLV2 = '".$EmpID['0']."' OR EmployeeCodeLV3 = '".$EmpID['0']."'   OR EmployeeCodeLV4 = '".$EmpID['0']."' OR EmployeeCodeLV5 = '".$EmpID['0']."' OR EmployeeCodeLV6 = '".$EmpID['0']."'
        OR ParentEmployeeCode = '".$EmpID['0']."')  )
 ";
  ...
  if (!empty($_REQUEST['EmpID'])) {
    ...
    if ($_REQUEST['EmpID'] == "7") {
      if (($_COOKIE['tsr_emp_permit'] == 1) || ($_COOKIE['tsr_emp_permit'] == 2) || ($_COOKIE['tsr_emp_permit'] == 6)
       || ($_COOKIE['tsr_emp_permit'] == 14) || ($_COOKIE['tsr_emp_permit'] == 7) || ($_COOKIE['tsr_emp_permit'] == 10) || ($_COOKIE['tsr_emp_permit'] == 13)) {
        $_REQUEST['EmpID'] = "A30970";
      }else {
        $_REQUEST['EmpID'] = "A".substr($_COOKIE['tsr_emp_id'],1,5);
      }
    }
      $WHERE .= "AND R.ZoneCode IN (SELECT DISTINCT SaleCode FROM [TSRData_Source].[dbo].[EmployeeDataParent_ALL]  WHERE StatusType = 'CRD' AND  (EmployeeCodeLV2 = '".$_REQUEST['EmpID']."' OR EmployeeCodeLV3 = '".$_REQUEST['EmpID']."' OR EmployeeCodeLV4 = '".$_REQUEST['EmpID']."' OR EmployeeCodeLV5 = '".$_REQUEST['EmpID']."' OR EmployeeCodeLV6 = '".$_REQUEST['EmpID']."' OR ParentEmployeeCode = '".$_REQUEST['EmpID']."')  )";
  }else {
    if (($_COOKIE['tsr_emp_permit'] == 1) || ($_COOKIE['tsr_emp_permit'] == 2) || ($_COOKIE['tsr_emp_permit'] == 6) || ($_COOKIE['tsr_emp_permit'] == 14) || ($_COOKIE['tsr_emp_permit'] == 7) || ($_COOKIE['tsr_emp_permit'] == 10)
 ...
 ?>
  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
      <div class="row">
        <form role="form" data-toggle="validator" id="formSearch" name="formSearch" method="post" action="index.php?pages=reportcrd2_bak">
        <div class="col-md-2">
          <h4>
            เก็บเงินงวดแรก
          </h4>
        ...
        <div class="col-xs-12">
          ...
          <!--<div class="box-body">-->
            ...
            <tbody>";
              ...
              ,ISNULL(ManualVolumeNo,'-') AS BookNo
              ,ISNULL(ManualRunningNo,'-') AS ReceiptNo
              from (SELECT DISTINCT ReceiptCode
              ,R.ReceiptID,B.ManualVolumeNo,B.ManualRunningNo
              ...
              $sql_body = " FROM TSRData_Source.dbo.vw_ReceiptWithZone AS R WITH(NOLOCK) LEFT JOIN Bighead_Mobile.dbo.Contract AS C WITH(NOLOCK) ON R.RefNo = C.RefNo LEFT JOIN [TSRData_Source].[dbo].[vw_DebtorCustomer] AS GC WITH(NOLOCK) ON C.CustomerID = GC.CustomerID LEFT JOIN SalePaymentPeriodPayment As Sy WITH(NOLOCK) ON R.PaymentID = Sy.PaymentID AND R.ReceiptID = Sy.ReceiptID LEFT JOIN Bighead_Mobile.dbo.SalePaymentPeriod AS S WITH(NOLOCK) ON S.SalePaymentPeriodID = Sy.SalePaymentPeriodID LEFT JOIN Bighead_Mobile.dbo.Employee AS Em WITH(NOLOCK) ON R.LastUpdateBy = EM.EmpID LEFT JOIN Bighead_Mobile.dbo.ManualDocument AS B ON B.DocumentNumber = R.ReceiptID AND ManualVolumeNo IS NOT NULL AND ManualRunningNo IS NOT NULL WHERE $WHERE AND S.SalePaymentPeriodID = Sy.SalePaymentPeriodID AND R.TypeCode = 1
              ) as result GROUP BY ReceiptCode,PaymentDueDate,CONTNO,CustomerName,EmpID,SaleCode,Names,Paydate,PrintName,RefNo,NetAmount,ReceiptID,ManualVolumeNo,ManualRunningNo ORDER BY ReceiptCode";
