...
if (empty($_REQUEST['searchDate'])) {
  ...
  $WHERE = "AND RV.LastUpdateDate BETWEEN '".DateEng($_REQUEST['searchDate'])." 00:00' AND '".DateEng($_REQUEST['searchDate'])." 23:59'";
}
...
if (empty($_REQUEST['startDate']) && empty($_REQUEST['endDate'])) {
  ...
  $d =  substr($_REQUEST['searchDate'],0,2);
  $dateSearch = "(".$d."/".$m."/".$y." - ".$d."/".$m."/".$y.")";
  $WHERE = "AND RV.LastUpdateDate BETWEEN '".DateEng($_REQUEST['startDate'])." 00:00' AND '".DateEng($_REQUEST['endDate'])." 23:59'";
}
  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
      <div class="row">
        ...
        <div class="col-md-3">
          <form role="form" data-toggle="validator" id="formSearch" name="formSearch" method="post" action="index.php?pages=reportcredit4">
            <?php
              if (($_COOKIE['tsr_emp_permit'] != 4 )) {
             ?>
        ...
        <div class="col-md-12">
          <?php
            if (!empty($_REQUEST['searchDate']) || ((!empty($_REQUEST['startDate']) && !empty($_REQUEST['endDate'])))) {
             ...
             FROM (SELECT CCode,Name,COUNT(RefNO) AS Ref,SUM(PAYAMT) as PAYAMT ,RefNO FROM (SELECT p.ZoneCode AS CCode,emd.EmployeeName AS Name,P.TotalPayment AS PAYAMT,P.DatePayment AS PayDate,P.RefNO
             FROM TSRDATA_Source.dbo.vw_ReceiptWithZone AS P INNER JOIN Bighead_Mobile.dbo.ReceiptVoid AS RV ON P.ReceiptID = RV.ReceiptID
             LEFT JOIN [Bighead_Mobile].[dbo].[EmployeeDetail] AS Emd ON P.LastUpdateBy = Emd.EmployeeCode WHERE Emd.SourceSystem = 'Credit' AND PositionCode = 'Credit' AND P.TotalPayment = 0 $WHERE ) as a GROUP BY CCode,Name,RefNO) as b  GROUP BY CCode,Name";
